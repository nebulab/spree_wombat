    ...
    module Handler
      class SetPriceHandler < Base
        def process
          ...
          current_currency = variant.cost_currency

          Spree::Variant.transaction do
            variant.update_attributes!(updatable_columns)
          end

          return response("Set price for #{sku} from #{current_price} #{current_currency} to #{variant.reload.price} #{variant.reload.cost_currency}")
        end
      end
    end
  end
